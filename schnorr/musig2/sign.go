...
var infinityPoint eclib.JacobianPoint
...
func NewPartialSignature(s *eclib.ModNScalar,
	r *eclib.PublicKey) PartialSignature {
...
func computeSigningNonce(combinedNonce [PubNonceSize]byte,
	combinedKey *eclib.PublicKey, msg [32]byte) (
	*eclib.JacobianPoint, *eclib.ModNScalar, error) {
	...
	//  * R = R_1 + b*R_2
	var nonce eclib.JacobianPoint
	eclib.ScalarMultNonConst(&nonceBlinder, &r2J, &r2J)
	eclib.AddNonConst(&r1J, &r2J, &nonce)
...
// infinity.
func Sign(secNonce [SecNonceSize]byte, privKey *eclib.PrivateKey,
	combinedNonce [PubNonceSize]byte, pubKeys []*eclib.PublicKey,
	...
	if !bytes.Equal(secNonce[eclib.PrivKeyBytesLen*2:],
	...
	var k1, k2 eclib.ModNScalar
	k1.SetByteSlice(secNonce[:eclib.PrivKeyBytesLen])
	k2.SetByteSlice(secNonce[eclib.PrivKeyBytesLen:])
	...
	nonceKey := eclib.NewPublicKey(&nonce.X, &nonce.Y)
	...
	parityCombinedKey := new(eclib.ModNScalar).SetInt(1)
...
func (p *PartialSignature) Verify(pubNonce [PubNonceSize]byte,
	combinedNonce [PubNonceSize]byte, keySet []*eclib.PublicKey,
	signingKey *eclib.PublicKey, msg [32]byte, signOpts ...SignOption) bool {
...
func verifyPartialSig(partialSig *PartialSignature, pubNonce [PubNonceSize]byte,
	...
	nonceMsgBuf.Write(msg[:])
	nonceBlindHash := ops.TaggedHash(NonceBlindTag, nonceMsgBuf.Bytes())
	nonceBlinder.SetByteSlice(nonceBlindHash[:])
	...
	pubNonce1J, err := eclib.ParseJacobian(
		pubNonce[:eclib.PubKeyBytesLenCompressed],
	)
	if err != nil {
		return err
	}
	pubNonce2J, err := eclib.ParseJacobian(
		pubNonce[eclib.PubKeyBytesLenCompressed:],
	)
	if err != nil {
		return err
	...
	var pubNonceJ eclib.JacobianPoint

	eclib.ScalarMultNonConst(&nonceBlinder, &pubNonce2J, &pubNonce2J)
	eclib.AddNonConst(&pubNonce1J, &pubNonce2J, &pubNonceJ)
	...
	//  * e = H(tag=ChallengeHashTag, R || Q || m) mod n
	var challengeMsg bytes.Buffer
	challengeMsg.Write(schnorr.SerializePubKey(eclib.NewPublicKey(
		&nonce.X, &nonce.Y,
	)))
	challengeMsg.Write(schnorr.SerializePubKey(combinedKey.FinalKey))
	challengeMsg.Write(msg[:])
	...
	signingKey, err := eclib.ParsePubKey(pubKey)
	if err != nil {
		return err
	}
	...
	var signKeyJ eclib.JacobianPoint
	signingKey.AsJacobian(&signKeyJ)

	// In the final set, we'll check that: s*G == R' + e*a*g*P.
	var sG, rP eclib.JacobianPoint
	eclib.ScalarBaseMultNonConst(s, &sG)
	eclib.ScalarMultNonConst(e.Mul(a).Mul(finalParityFactor), &signKeyJ, &rP)
	eclib.AddNonConst(&rP, &pubNonceJ, &rP)
...
func WithTweakedCombine(msg [32]byte, keys []*eclib.PublicKey,
...
func WithTaprootTweakedCombine(msg [32]byte, keys []*eclib.PublicKey,
...
func WithBip86TweakedCombine(msg [32]byte, keys []*eclib.PublicKey,
...
func CombineSigs(combinedNonce *eclib.PublicKey,
	...
	var tweakProduct *eclib.ModNScalar
	if opts.combinedKey != nil && opts.tweakAcc != nil {
		...
		parityFactor := new(eclib.ModNScalar).SetInt(1)
